...
const THEME_KEYS = new Set<string>(["name", "tokens", "fontFamilies"]);

const TOKEN_KEYS = new Set<keyof ThemeTokens>([
  "colorSurface",
  "colorPanel",
  "colorPrimary",
  "colorText",
...
const LIFECYCLE_KEYS = new Set<keyof LifecycleHooks>([
  "onInitialize",
  "onMessageSent",
  "onStreamUpdate",
  "onToggleLayout",
...
const STORAGE_KEYS = new Set<keyof StorageConfig>([
  "enabled",
  "key",
  "maxMessages",
  "persistOpenState"
]);
const PRESENCE_KEYS = new Set<keyof PresenceConfig>([
  "enabled",
  "intervalMs",
  "pauseWhenHidden",
  "pollWhenClosed"
...
const I18N_MESSAGE_KEYS = new Set<keyof I18nMessages>([
...
const POSITION_VALUES = new Set<WidgetPosition>([
  "bottom-right",
  "bottom-left",
  "bottom-center",
  "top-right",
...
function assertMode(value: unknown, path: string): asserts value is WidgetMode {
  if (value !== "normal" && value !== "landscape") {
    throw new Error(`${path} must be either \"normal\" or \"landscape\".`);
  }
}

function assertPosition(value: unknown, path: string): asserts value is WidgetPosition {
  if (typeof value !== "string" || !POSITION_VALUES.has(value as WidgetPosition)) {
...
function validatePartialTokens(value: unknown, path: string): asserts value is Partial<ThemeTokens> {
...
function validateTeaserInput(value: unknown, path: string): asserts value is Partial<TeaserConfig> {
  assertRecord(value, path);
...
function validateStorageInput(value: unknown, path: string): asserts value is Partial<StorageConfig> {
  assertRecord(value, path);
...
function validatePresenceInput(value: unknown, path: string): asserts value is Partial<PresenceConfig> {
  assertRecord(value, path);
...
function validateThinkingInput(value: unknown, path: string): asserts value is Partial<ThinkingConfig> {
  assertRecord(value, path);
...
function validateActionHandlersMap(
  value: unknown,
  path: string
): asserts value is Partial<ActionHandlers> {
  assertRecord(value, path);
...
function validateI18nMessages(
  value: unknown,
  path: string
): asserts value is Partial<I18nMessages> {
  assertRecord(value, path);
...
function validateI18nInput(value: unknown, path: string): asserts value is I18nConfigInput {
  assertRecord(value, path);
...
export function validatePartialTheme(value: unknown, path = "theme"): asserts value is ThemeInput {
  assertRecord(value, path);
...
export function validateLifecycleMap(
  value: unknown,
  path = "lifecycle"
): asserts value is Partial<LifecycleHooks> {
  assertRecord(value, path);
...
export function validateConfigInput(value: unknown, path = "config"): asserts value is ConfigInput {
  assertRecord(value, path);
...
export function validateResolvedConfig(config: Config): void {
